...
function renderLocation(data: AppointmentConfirmationData): string {
  ...
  const meetLinkHtml = data.meetLink
    ? `<p style="margin:8px 0 0;font-size:14px;"><a href="${escapeHtml(data.meetLink)}" style="color:${COLORS.primary};text-decoration:underline;">Unirse a la videollamada</a></p>`
    : ''
...
function renderTransferData(transferData: TransferData, price: number): string {
  return `
    <div style="background-color:${COLORS.accentFaint};border-radius:8px;padding:20px;margin-bottom:8px;">
      <p style="margin:0 0 12px;font-size:15px;font-weight:600;color:${COLORS.text};">Datos para transferencia (${formatCLP(price)})</p>
      <table role="presentation" style="width:100%;border-collapse:collapse;font-size:14px;">
        <tr><td style="padding:4px 0;color:${COLORS.muted};width:140px;">RUT</td><td style="padding:4px 0;font-weight:600;">${escapeHtml(transferData.rut)}</td></tr>
        <tr><td style="padding:4px 0;color:${COLORS.muted};">Banco</td><td style="padding:4px 0;font-weight:600;">${escapeHtml(transferData.bank)}</td></tr>
        <tr><td style="padding:4px 0;color:${COLORS.muted};">Tipo de cuenta</td><td style="padding:4px 0;font-weight:600;">${escapeHtml(transferData.accountType)}</td></tr>
        <tr><td style="padding:4px 0;color:${COLORS.muted};">N° de cuenta</td><td style="padding:4px 0;font-weight:600;">${escapeHtml(transferData.accountNumber)}</td></tr>
        <tr><td style="padding:4px 0;color:${COLORS.muted};">Email</td><td style="padding:4px 0;font-weight:600;">${escapeHtml(transferData.email)}</td></tr>
      </table>
    </div>`
}
  ...
  const bodyHtml = `
    ...
    <div style="background-color:${COLORS.primaryFaint};border-radius:8px;padding:20px;margin-bottom:24px;">
      <table role="presentation" style="width:100%;border-collapse:collapse;font-size:15px;">
        ...
        <tr><td style="padding:6px 0;color:${COLORS.muted};">Hora</td><td style="padding:6px 0;font-weight:600;">${escapeHtml(data.time)}</td></tr>
