...
export function paymentReminderTemplate(data: PaymentReminderData): EmailTemplate {
  ...
  const bodyHtml = `
    ...
    <div style="background-color:${COLORS.accentFaint};border-radius:8px;padding:20px;margin-bottom:24px;">
      <p style="word-wrap:break-word;overflow-wrap:break-word;max-width:100%;margin:0 0 12px;font-size:15px;font-weight:600;color:${COLORS.text};">Datos para transferencia (${formatCLP(data.price)})</p>
    ...
    <p style="word-wrap:break-word;overflow-wrap:break-word;max-width:100%;margin:0 0 16px;font-size:15px;">Una vez realizada la transferencia, puedes enviarnos el comprobante por WhatsApp:</p>
