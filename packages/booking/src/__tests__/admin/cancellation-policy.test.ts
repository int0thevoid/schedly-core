...
const DOCUMENT = {
  ...
  content: '<ul><li>Contenido por defecto.</li></ul>',
  updatedById: null,
  createdAt: new Date(),
  updatedAt: new Date(),
}
...
describe('GET /api/cancellation-policy (public)', () => {
  ...
  it('falls back to the default text when no document has been configured yet', async () => {
    ...
    const res = await request(app).get('/api/cancellation-policy')
    expect(res.status).toBe(200)
    expect(res.body.data.title).toBe('Política de cancelación')
    expect(res.body.data.content).toContain('Puedes reagendar con mínimo 24 horas de anticipación')
  })
  ...
  it('queries by the cancellation type, not the data storage document', async () => {
    prismaMock.consentDocument.findUnique.mockResolvedValue(null)
    await request(app).get('/api/cancellation-policy')
    expect(prismaMock.consentDocument.findUnique).toHaveBeenCalledWith({
      where: { professionalId_type: { professionalId: 'pro1', type: 'cancellation' } },
    })
  })
})

describe('GET /api/admin/cancellation-policy', () => {
  it('returns 401 without token', async () => {
    const res = await request(app).get('/api/admin/cancellation-policy')
    expect(res.status).toBe(401)
  })

  it('returns the existing document', async () => {
    prismaMock.consentDocument.upsert.mockResolvedValue(DOCUMENT)
    const res = await request(app).get('/api/admin/cancellation-policy').set('Cookie', `auth_token=${token()}`)
    expect(res.status).toBe(200)
  ...
  it('creates a default document lazily when none exists yet (atomically, via upsert)', async () => {
    ...
    expect(prismaMock.consentDocument.upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        create: expect.objectContaining({ professionalId: 'pro1', type: 'cancellation' }),
      }),
    )
  })
})

describe('PATCH /api/admin/cancellation-policy', () => {
  it('updates title and content', async () => {
    ...
    const res = await request(app)
      .patch('/api/admin/cancellation-policy')
